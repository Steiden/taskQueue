taskQueue.grid.Default = function (config) {
    config = config || {};
    ...
    Ext.applyIf(config, {
        ...
        baseParams: {},
        cls: config['cls'] || 'main-wrapper taskqueue-grid',
        autoHeight: true,
        paging: true,
        remoteSort: true,
        ...
        viewConfig: {
            ...
            getRowClass: function (rec) {
                ...
                if (rec.data['published'] != undefined && rec.data['published'] == 0) {
                    cls.push('taskqueue-row-unpublished');
                }
                if (rec.data['active'] != undefined && rec.data['active'] == 0) {
                    cls.push('taskqueue-row-inactive');
                }
                if (rec.data['deleted'] != undefined && rec.data['deleted'] == 1) {
                    cls.push('taskqueue-row-deleted');
                }
                if (rec.data['required'] != undefined && rec.data['required'] == 1) {
                    cls.push('taskqueue-row-required');
                }
                return cls.join(' ');
            }
        },
    });
    taskQueue.grid.Default.superclass.constructor.call(this, config);
...
Ext.extend(taskQueue.grid.Default, MODx.grid.Grid, {
    ...
    getMenu: function (grid, rowIndex) {
        ...
        var menu = taskQueue.utils.getMenu(row.data['actions'], this, ids);
        ...
        new Ext.dd.DropTarget(el, {
                    ...
                    listeners: {
                        success: {
                            fn: function () {
                                ...
                                if (grid.xtype == 'taskqueue-grid-products' && !grid.defaultNotify) {
                                        ...
                                        if (singleParent) {
                                            if (sourceNodes[0].data.parent != target.data.parent) {
                                                if (target.data.category_name == '') {
                                                    message = (sourceNodes.length > 1) ? _('taskqueue_drag_move_current_many_success') : _('taskqueue_drag_move_current_once_success');
                                                } else {
                                                    message = (sourceNodes.length > 1) ? String.format(_('taskqueue_drag_move_many_success'), target.data.category_name) : String.format(_('taskqueue_drag_move_one_success'), target.data.category_name);
                                                }
                                            }
                                            // else {
                                            //     message = (sourceNodes.length > 1) ? _('taskqueue_drag_sort_many_success') : _('taskqueue_drag_sort_once_success');
                                            // }
                                        } else {
                                            message = (sourceNodes.length > 1) ? String.format(_('taskqueue_drag_move_many_success'), target.data.category_name) : String.format(_('taskqueue_drag_move_one_success'), target.data.category_name);
                                        }
            ...
            _notifyMove: function (count, targetNode, dd) {
                ...
                if (targetNode.data.category_name == '') {
                    dd.ddel.innerHTML = (count > 1) ? _('taskqueue_drag_move_current_many') : _('taskqueue_drag_move_current_one');
                } else {
                    dd.ddel.innerHTML = (count > 1) ? String.format(_('taskqueue_drag_move_many'), targetNode.data.category_name) : String.format(_('taskqueue_drag_move_one'), targetNode.data.category_name);
                }
            },
            _notifySort: function (count, dd) {
                returnCls = 'x-tree-drop-ok-between';
                dd.ddel.innerHTML = (count > 1) ? _('taskqueue_drag_sort_many') : _('taskqueue_drag_sort_one');
            },
            _notifySelf: function (count, dd) {
                dd.ddel.innerHTML = (count > 1) ? _('taskqueue_drag_self_many') : _('taskqueue_drag_self_one');
            }
        });
    },
...
Ext.reg('taskqueue-grid-default', taskQueue.grid.Default);